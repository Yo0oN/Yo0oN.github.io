---
title: 분산 처리 환경에서 데이터 불일치를 해결하는 방법 1편 (Sticky Session, Session Clustering)
modified: 2020-10-24
author: Yo0oN
...
In-memory Database는 HDD나 SSD가 아닌 주기억장치(RAM)에 저장되는 데이터베이스로, 기존의 방식들과는 다르게 세션 저장소를 따로 분리해서 사용하는 것이다.<br>
디스크가 아닌 메모리에 저장되기 때문에 데이터를 접근할 때 더 빠르다는 장점이 있다.

하지만 휘발성 데이터이기 때문에 컴퓨터의 전원이 꺼지면 데이터도 사라진다....
추후 보강
